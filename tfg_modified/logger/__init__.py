...
# from .molecule_logger import MoleculeLogger  # DELETED
# from .audio_logger import AudioLogger  # DELETED
...
def setup_logger(args) -> Union[BaseLogger, ImageLogger]:
    """
    FIXED: Setup logger with only remaining logger types
    """
    dir = os.path.expanduser(args.logging_dir)
    log_suffix = args.log_suffix
    os.makedirs(os.path.expanduser(dir), exist_ok=True)
    ...
    if args.data_type == 'image':
        ...
        # Default to base logger for unsupported types
        print(f"Warning: Data type '{args.data_type}' not explicitly supported, using BaseLogger")
        _logger = BaseLogger(args, output_formats)
